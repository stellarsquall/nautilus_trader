...
export class VolumeProfileOverlay {
  ...
  /**
   * Clear the overlay's own canvas layer. The overlay draws to a dedicated
   * canvas, so simply skipping `render()` (e.g. when the Volume Profile toggle
   * is off) leaves the last-drawn bars frozen on screen. Callers must clear
   * this layer explicitly when hiding the overlay.
   */
  public clear(): void {
