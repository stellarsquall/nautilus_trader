...
@pytest.fixture
def actor_config():
    return BarStreamingActorConfig(price_bin_size=0.1)
...
def test_footprint_custom_bin_size(actor, mock_queue, mock_loop):
    ...
    config = BarStreamingActorConfig(price_bin_size=0.5)
    ...
    # bin_index = floor(price / bin_size) per spec: floor(100.4/0.5)=floor(200.8)=200,
    # level_price = 200 * 0.5 = 100.0
    for level in payload["levels"]:
