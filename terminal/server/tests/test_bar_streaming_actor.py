...
def test_trade_bucketing(actor):
    ...
    assert bin_data["buy_volume"] == pytest.approx(10.5, abs=0.01)
    assert bin_data["sell_volume"] == pytest.approx(6.2, abs=0.01)
    assert bin_data["buy_volume"] - bin_data["sell_volume"] == pytest.approx(4.3, abs=0.01)
...
def test_trade_bucketing_keys_by_minute(actor):
    ...
    assert actor._buckets[m0][1000]["buy_volume"] == pytest.approx(1.0)
    assert actor._buckets[m0][1000]["sell_volume"] == pytest.approx(2.0)
    assert actor._buckets[m1][1000]["buy_volume"] == pytest.approx(3.0)
...
def test_footprint_envelope_emission(actor, mock_queue, mock_loop):
    ...
    assert minute_bucket[1000]["buy_volume"] == pytest.approx(3.0)   # 2.0 + 1.0
    assert minute_bucket[1000]["sell_volume"] == pytest.approx(3.0)  # 3.0
    assert minute_bucket[1005]["buy_volume"] == pytest.approx(1.5)
    assert minute_bucket[1005]["sell_volume"] == pytest.approx(0.0)
    assert minute_bucket[1010]["buy_volume"] == pytest.approx(0.0)
    assert minute_bucket[1010]["sell_volume"] == pytest.approx(0.5)
    ...
    # Verify levels sorted by bin index
    bin_indices = [l["bin"] for l in payload["levels"]]
    assert bin_indices == sorted(bin_indices)
    assert bin_indices == [1000, 1005, 1010]

    # Verify per-bin volumes
    level_map = {l["bin"]: l for l in payload["levels"]}
